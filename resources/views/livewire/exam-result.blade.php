<div class="mx-auto max-w-4xl p-6 space-y-6">
    <h1 class="text-xl font-medium text-gray-900">Exam Result: {{ $exam->title }}</h1>
    ...
    @if($ok)
        ...
        <div class="rounded-xl shadow-lg p-6 bg-gradient-to-r from-indigo-50 via-sky-50 to-purple-50 border border-indigo-100">
            <div class="flex items-center gap-3">
                @if($passed)
                    ...
                    <div>
                        <div class="text-lg font-semibold text-green-700">Passed</div>
                        <div class="text-sm text-gray-700">Score: {{ number_format($stats['percentage'] ?? 0, 1) }}% - The passing threshold for this exam was {{ number_format($passThreshold ?? 50, 0) }}%.</div>
                    </div>
                @else
                    @svg('heroicon-o-x-circle', 'w-10 h-10 text-rose-600')
                    <div>
                        <div class="text-lg font-semibold text-rose-700">Failed</div>
                        <div class="text-sm text-gray-700">Score: {{ number_format($stats['percentage'] ?? 0, 1) }}% - The passing threshold for this exam was {{ number_format($passThreshold ?? 50, 0) }}%.</div>
                    </div>
                @endif
            </div>
            ...
            <div class="mt-5 grid grid-cols-3 gap-3 text-center">
                ...
                <div class="rounded-lg p-4 bg-gradient-to-br from-rose-50 to-pink-50 border border-rose-100">
                    <div class="text-xs text-rose-700">Wrong</div>
                ...
                <div class="rounded-lg p-4 bg-gradient-to-br from-amber-50 to-yellow-50 border border-amber-100">
                    <div class="text-xs text-amber-700">Unanswered</div>
        ...
        <!-- Questions Review -->
        <div class="space-y-4">
            <h2 class="text-lg font-medium text-gray-900">Question Review (Answered questions only)</h2>
            @foreach($review as $item)
                <div class="rounded-xl shadow p-6 bg-gradient-to-r from-white to-slate-50 {{ !empty($item['is_correct']) ? 'border-2 border-green-200' : 'border-2 border-rose-200' }}">
                    ...
                    <div class="flex items-center justify-between mb-4">
                        <h3 class="text-lg font-bold text-gray-900">Question {{ $item['question_number'] }}</h3>
                    ...
                    @if(!empty($item['is_deleted']))
                        <div class="rounded-lg border border-amber-200 bg-amber-50 p-4 mb-4">
                            <p class="text-sm text-amber-700">
                                <strong>Note:</strong> This question has been officially removed and was not calculated in your final score.
                            </p>
                        </div>
                    @endif
                    ...
                    @if(!empty($item['user_choice_text']))
                        <div class="rounded-lg border p-4 bg-gradient-to-r from-sky-50 to-blue-50 border-sky-200 mb-2">
                            <div class="flex items-start gap-3">
                                ...
                                @endif
                                <div class="flex-1">
                                    <span class="font-bold text-gray-900">Your Answer: Option {{ $item['user_no'] }}</span>
                    ...
                    @if(!empty($item['correct_choice_text']))
                        <div class="rounded-lg border-2 p-4 bg-green-50 border-green-200">
                            <div class="flex items-start gap-3">
                                @svg('heroicon-o-check-circle', 'w-6 h-6 text-green-600 flex-shrink-0 mt-1')
                                <div class="flex-1">
                                    <span class="font-bold text-gray-900">Correct Answer: Option {{ $item['correct_no'] }}</span>
                                    <div class="mt-1 prose prose-sm max-w-none text-gray-800">
    ...
    @else
        <div class="rounded border p-4 text-sm text-gray-600">No results found to display.</div>
    @endif

    <div class="flex justify-center">
        <a href="{{ route('home') }}" wire:navigate class="inline-flex items-center rounded bg-indigo-600 px-4 py-2 text-white text-sm font-medium hover:bg-indigo-700 transition">Back to Home</a>
    </div>
</div>
